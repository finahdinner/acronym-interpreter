...
def main() -> str:
    ...
    while True:
        ...
        if not (sentence_structure := select_sentence_structure(acronym)):
            sys.exit("Please try an acronym that is 2-5 letters long.")
        ...
        # initialise with None as each value, to track whether it has been processed
        processed_words = [None for _ in unprocessed_words]
        ...
        if sentence_makes_sense(finished_sentence):
            return f"Meaning: {finished_sentence}"
        else:
            continue
